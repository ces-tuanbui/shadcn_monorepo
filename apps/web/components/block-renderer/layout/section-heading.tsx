...
import type { SectionHeadingProps } from "@workspace/ui/types";
...
export function SectionHeading(data: Readonly<SectionHeadingProps>) {
  ...
  return (
    <div
      className={cn(
        "container items-center justify-between gap-2 py-4",
        headingStyle
      )}
    >
      <div className="flex flex-col gap-3">
        <span className="font-bold uppercase text-primary">{subHeading}</span>
        <h2 className="font-heading text-3xl font-semibold sm:text-4xl mb-2">
          {heading}
        </h2>
      </div>
